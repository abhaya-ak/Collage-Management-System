...
from apps.core.enums import AuditEvent, FacultyStatus, LeaveStatus, UserRole
...
# =============================================================
@transaction.atomic
def request_leave(*, faculty, start_date, end_date, reason="", actor=None):
    """Create a PENDING leave request for a faculty."""
    if end_date < start_date:
        raise ValidationException("End date cannot be before start date.")
    leave = FacultyLeave.objects.create(
        faculty=faculty, start_date=start_date, end_date=end_date, reason=reason,
    )
    log_audit(action=AuditEvent.LEAVE_REQUESTED, actor=actor, instance=leave,
              metadata={"faculty": faculty.employee_id})
    return leave


@transaction.atomic
def approve_leave(leave, actor=None):
    """Approve a leave (admin). Also flips faculty status to ON_LEAVE."""
    if leave.status != LeaveStatus.PENDING:
        raise InvalidOperation(f"Leave is already {leave.status}.")
    leave.status = LeaveStatus.APPROVED
    leave.approved_by = actor if getattr(actor, "pk", None) else None
    leave.approved_at = timezone.now()
    leave.save(update_fields=["status", "approved_by", "approved_at", "updated_at"])
    log_audit(action=AuditEvent.LEAVE_APPROVED, actor=actor, instance=leave,
              metadata={"faculty": leave.faculty.employee_id})
    return leave


@transaction.atomic
def reject_leave(leave, actor=None):
    """Reject a pending leave (admin)."""
    if leave.status != LeaveStatus.PENDING:
        raise InvalidOperation(f"Leave is already {leave.status}.")
    ...
    leave.approved_at = timezone.now()
    leave.save(update_fields=["status", "approved_by", "approved_at", "updated_at"])
    log_audit(action=AuditEvent.LEAVE_REJECTED, actor=actor, instance=leave,
              metadata={"faculty": leave.faculty.employee_id})
    return leave
