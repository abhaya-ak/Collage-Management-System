...
class RoutineViewSet(BaseRBACViewSet):
    serializer_class = serializers.RoutineSerializer
    permission_map = perms.ROUTINE_PERMISSIONS
    filterset_fields = ["faculty_assignment", "day_of_week"]

    def get_queryset(self):
        return Routine.objects.select_related(
            "faculty_assignment__subject",
            "faculty_assignment__faculty",
            "faculty_assignment__section",
        )


class AcademicLeaveViewSet(BaseRBACViewSet):
    serializer_class = serializers.AcademicLeaveSerializer
    permission_map = perms.ACADEMIC_LEAVE_PERMISSIONS
    filterset_fields = ["academic_year"]
    ordering_fields = ["start_date"]

    def get_queryset(self):
        return AcademicLeave.objects.select_related("academic_year")
